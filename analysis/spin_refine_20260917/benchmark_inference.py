"""Read-only refinement inference costs: matched one/two steps, historical model, CPU Linear.
...
METHODS = ("one_step", "two_step", "spin_direct")
...
def records(args):
    ...
    for method in METHODS:
        ...
        run_root = args.repo_root / "outputs/spin-path-controls-20260917-v1" if method == "spin_direct" else args.run_root
        protocol = "spin-path-controls-development-20260917-v1" if method == "spin_direct" else PROTOCOL
        job = run_root / args.dataset / method / "seed41001"
        path = job / "best.pt"
        config_path, result_path = job / "config.json", job / "result.json"
        config, result = common.read(config_path), common.read(result_path)
        identity = {"protocol": protocol, "dataset": args.dataset, "variant": method,
...
def load_model(method, record, flows, fit):
    import torch
    if method == "spin_direct":
        from experiments.spin_path_controls_v1.models import build_model
    else:
        from experiments.spin_refine_v1.models import build_model
...
def benchmark(args, output):
    ...
    from experiments.spin_refine_v1 import models
    common.check_stop(args)
    ...
    values, starts, fit, neighbors_cpu = data.load_fit_windows(args.dataset, indices=tuple(range(8)))
    ...
    fit_identity = {"stats_sha256": data._arrays_sha256(fit_arrays),
                    ...
                    "train_starts_sha256": data.array_sha256(data.window_starts(args.dataset, "fit"))}
    ...
    if values.shape != (8, 50, 462 if args.dataset == "geant" else 144):
        raise ValueError("Expected original first-eight fit-window dimensions")
    families = [data.make_mask_family(flows, int(start), 71001, dataset=args.dataset) for start in starts]
    ...
    record = {"fit_identity": fit_identity, "window_indices": list(range(8)),
              ...
              "models": {}, "measurements": []}
    output["datasets"][args.dataset] = record
            ...
            for condition in data.CONDITIONS:
                def once():
                    return engine.predict_windows(model, values[:batch], masks[condition][:batch],
                        starts[:batch], stats, neighbors, dataset=args.dataset,
                ...
                print(f"{args.dataset}/{method}/B{batch}/{condition}: {row['median_ms_per_batch']:.3f} ms", flush=True)
                del final, raw
        del model
        gc.collect()
    ...
    for batch in (1, 8):
        for condition in data.CONDITIONS:
            ...
            record["measurements"].append(row)
            common.write(args.output, output)
            print(f"{args.dataset}/linear_cpu/B{batch}/{condition}: {row['median_ms_per_batch']:.3f} ms", flush=True)
            del final
...
def main():
    ...
    parser.add_argument("--run-root", type=Path, required=True,
                        help="Parent of {dataset}/{variant}/seed41001 completed run directories")
    ...
    output = {"state": "running", "role": "fit_only_inference_benchmark", "datasets": {},
        ...
        "protocol": {"precision": "FP32; TF32 disabled", "batches": [1, 8],
            ...
            "fit_input": "selected WAN first 8 fit windows; B1 uses the first of those windows",

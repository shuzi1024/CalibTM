    ...
    with lock_path.open('a') as lock:
        ...
        for variant in args.variants.split(','):
            ...
            command = [str(ROOT / '.venv/bin/python'), '-u', '-m',
                       'experiments.spin_refine_v1.runner',
...
def main():
    ...
    assert set(args.variants.split(',')) <= {'one_step', 'two_step'}
    auth = json.loads((HERE / 'AUTHORIZATION.json').read_text())
    assert auth['state'] == 'authorized' and auth['max_concurrent_gpus'] <= 4
    assert socket.gethostname() in auth['hosts']
    assert auth['started_unix'] <= time.time() < auth['deadline_unix'] - 300

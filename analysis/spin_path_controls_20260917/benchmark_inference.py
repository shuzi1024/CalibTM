"""Read-only GEANT inference costs: three path controls and CPU Linear.
...
def benchmark(args, output):
    ...
    values, starts, fit, neighbors_cpu = data.load_fit_windows("geant", indices=tuple(range(8)))
    ...
    fit_identity = {"stats_sha256": data._arrays_sha256(fit_arrays),
                    ...
                    "train_starts_sha256": data.array_sha256(data.window_starts("geant", "fit"))}
    ...
    if values.shape != (8, 50, 462):
        raise ValueError("Expected original GEANT first-eight fit-window dimensions")
    families = [data.make_mask_family(flows, int(start), 71001, dataset="geant") for start in starts]
            ...
            for condition in data.CONDITIONS:
                def once():
                    return engine.predict_windows(model, values[:batch], masks[condition][:batch],
                        starts[:batch], stats, neighbors, dataset="geant",
                ...
                print(f"geant/{method}/B{batch}/{condition}: {row['median_ms_per_batch']:.3f} ms", flush=True)
                del final, raw
        del model
        gc.collect()
    ...
    for batch in (1, 8):
        for condition in data.CONDITIONS:
            ...
            record["measurements"].append(row)
            common.write(args.output, output)
            print(f"geant/linear_cpu/B{batch}/{condition}: {row['median_ms_per_batch']:.3f} ms", flush=True)
            del final
...
def main():
    ...
    parser.add_argument("--run-root", type=Path, required=True,
                        help="Parent of geant/{variant}/seed41001 completed run directories")
    ...
    output = {"state": "running", "role": "fit_only_inference_benchmark", "datasets": {},
        ...
        "protocol": {"precision": "FP32; TF32 disabled", "batches": [1, 8],
            ...
            "fit_input": "GEANT first 8 fit windows; B1 uses the first of those windows",

"""Immutable source, protocol and permitted-data identity for ACIL mechanism v1."""
...
DEFAULT_FREEZE_PATH = _PACKAGE_ROOT / "freezes/acil_mechanism_v1.json"
_SOURCE_NAMES = (
    "__init__.py",
    "adjudicate.py",
    "freeze.py",
...
_TREE_DOMAIN = b"acil-mechanism-v1:source-tree:v1\x00"
...
def _current_record() -> dict[str, Any]:
    source_files = [
        _file_record(f"experiments/acil_mechanism_v1/{name}")
    ...
    return {
        ...
        "expected_jobs": {
            stage: list(expected_job_identities(stage))
            for stage in ("prototype", "extension")
        },
        "git_available": False,
        "git_commit": None,

"""Two SPIN task-adapted comparisons with official internal optimization defaults."""
...
from experiments.spin_comparison_v1.models import (build_model as build_spin,
    train_step as spin_train_step, make_optimizer_scheduler, MODEL_CONFIG, TRAINING as SPIN_TRAINING)
...
VARIANTS = ALL_VARIANTS = ("spin_adapted",)
SEED = 41001
PROTOCOL = "spin-adapted-development-20260914-v1"
...
CPU_CHECK = ROOT / "analysis/spin_comparison_20260914/CPU_CHECKS.json"
DEADLINE_UNIX = None  # New authorization; bounded by two jobs, 300 epochs and existing early stop.
STOP_REQUESTED = False
TRAINING = {
    "max_epochs": 300, "effective_batch": 8, "physical_batch": 8,
    "target_block": "full", "evaluation_batch": 8, "evaluation_target_block": "full",
    "optimizer": "Adam", "lr": 8e-4, "betas": [0.9, 0.999],
    "weight_decay": 0.0, "gradient_clip": 5.0,
    "scheduler": {"name": "CosineSchedulerWithRestarts", "warmup_epochs": 12,
        "min_factor": 0.1, "linear_decay": 0.67, "num_cycles": 3, "interval": "epoch"},
    "node_chunk": 32, "gradient_checkpointing": True,
    "initialization_seed": 41001, "data_order_seed": 51001,
    "training_mask_seed": 61001, "selection_mask_seed": 71001,
    "early_stop_patience_starts_epoch": 1, "early_stop_patience": 40,
    "earliest_stop_epoch": 1, "improvement": "strictly smaller selection score",
    "loss": "sum of four readout standardized missing L1 means; global fit scaler; original deep supervision",
...
def sources():
    paths = sorted((ROOT / "experiments/spin_comparison_v1").glob("*.py"))
    paths += [Path(__file__), Path(__file__).with_name("__init__.py"),
        ROOT / "experiments/spin_comparison_v1/source_manifest.json",
        ROOT / "experiments/spin_comparison_v1/README_CN.md",
        ROOT / "experiments/llm_candidates_runtime_v1/run.py",
        ROOT / "plan/CALIBTM_SPIN_EXECUTION_2026-09-14_CN.md",
        ROOT / "setup/ksc_spin_comparison_v1.sh"]
    return {str(p.relative_to(ROOT)): file_sha(p) for p in paths}


def build_model(num_flows, init_seed):
    model = build_spin(num_flows, init_seed=init_seed)
    model.set_execution(node_chunk=TRAINING["node_chunk"],
...
def prepare(args):
    ...
    if cpu_check.get("passed") is not True:
        raise RuntimeError("SPIN official-source equivalence checks missing or failed")
    ...
    for dataset in DATASETS:
        ...
        normalization = model.configure_from_bundle(bundle)
        state = cpu_state(model)
        count = sum(p.numel() for p in model.parameters())
        if count != cpu_check["parameter_counts"][str(bundle.flows)]:
            raise RuntimeError("CPU checked parameter count differs")
        relative = f"initializations/{dataset}/spin_adapted/seed{SEED}.pt"
        save_torch(out / relative, state)
        initializations[dataset] = {"spin_adapted": {str(SEED): {
            "file": relative, "sha256": file_sha(out / relative), "state_sha256": state_sha(state),
            "parameter_count": count, "normalization": normalization}}}
    record = {"protocol": PROTOCOL, "created": utc(), "parent": str(parent),
        ...
        "training": TRAINING, "adapter_training_defaults": SPIN_TRAINING, "model_config": MODEL_CONFIG,
        "runtime": runtime(), "source_files": sources(), "data": data, "references": references,
        "initializations": initializations, "deadline_unix": DEADLINE_UNIX,
        "scope": "one official-architecture SPIN task adapter x two WANs x seed41001; no grid",
        "maximum_formal_new_jobs": 2, "role": "development_only",
        "cpu_correctness": {"path": str(CPU_CHECK), "sha256": file_sha(CPU_CHECK)},
        "authorization": "User authorized next-step work and GPUs on 2026-09-14",
        "adaptations": ["original fit-only graph from common protocol", "T50 relative native-grid covariate",
            "common 20% masks and raw missing-only scoring", "epoch selection on common mean NMAE",
            "physical batch8 with exact checkpointed node chunks; effective batch8 unchanged"]}
...
def verify_registry(out):
    ...
    if (record["adapter_training_defaults"] != SPIN_TRAINING or record["model_config"] != MODEL_CONFIG):
        raise RuntimeError("SPIN architecture or optimizer defaults changed")
    if (file_sha(record["cpu_correctness"]["path"]) != record["cpu_correctness"]["sha256"]
            or read_json(record["cpu_correctness"]["path"]).get("passed") is not True):
        raise RuntimeError("CPU equivalence report changed")
...
def setup_run(args, record):
    ...
    model = build_model(bundle.flows, init_seed=SEED)
    normalization = model.configure_from_bundle(bundle)
    if normalization != initial["normalization"]:
        raise RuntimeError("fit-only normalization changed")
    ...
    optimizer._calibtm_scheduler = scheduler
...
def run_step(model, optimizer, bundle, stats, neighbors, order, masks, offset, epoch):
    ...
    return spin_train_step(model, bundle.train_windows[indices], masks[offset:offset + len(indices)],
...
def load_resume(path, identity, model, optimizer):
    ...
    optimizer._calibtm_scheduler.load_state_dict(checkpoint["scheduler"])
...
def smoke(args):
    ...
    with job_lock(destination / "active.lock"):
        ...
        save_torch(destination / "resume_probe.pt", {**identity, "model": cpu_state(model),
            "optimizer": optimizer.state_dict(), "scheduler": optimizer._calibtm_scheduler.state_dict(), "rng": capture_rng()})
        ...
        optimizer._calibtm_scheduler.step()  # perturb it to verify restoration
        assert optimizer._calibtm_scheduler.last_epoch == 1
        load_resume(destination / "resume_probe.pt", identity, model, optimizer)
        if optimizer._calibtm_scheduler.last_epoch != 0:
            raise RuntimeError("scheduler restore differs")
...
def train_locked(args, record, identity, job, report_path):
    ...
    for epoch in range(len(history), TRAINING["max_epochs"]):
        ...
        for offset in range(next_offset, len(order), TRAINING["effective_batch"]):
            if stopping():
                ...
                save_torch(job / "last.pt", {**identity, "model": cpu_state(model),
                    "optimizer": optimizer.state_dict(), "scheduler": optimizer._calibtm_scheduler.state_dict(), "rng": capture_rng(), "history": history,
        ...
        optimizer._calibtm_scheduler.step()
        row = {"learning_rate": epoch_lr, "next_learning_rate": optimizer.param_groups[0]["lr"], "epoch": epoch + 1, "updates": (epoch + 1) * 64,
...
def check_job_allowed(args):
    if VARIANT != "spin_adapted" or SEED != 41001 or args.dataset not in DATASETS:
        raise RuntimeError("outside two registered SPIN jobs")
...
def main():
    ...
    VARIANT, SEED = getattr(args, "variant", "spin_adapted"), getattr(args, "seed", 41001)
